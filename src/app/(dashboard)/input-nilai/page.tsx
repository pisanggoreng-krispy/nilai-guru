...
export default function InputNilaiPage() {
  ...
  // Filter subjects by jenjang AND grade matching the selected class
  const filteredSubjects = subjects.filter(s => {
    if (!selectedClassId) return false;
    const selectedClass = classes.find(c => c.id === selectedClassId);
    if (!selectedClass) return false;
    return s.jenjang === selectedClass.jenjang && s.grade === selectedClass.grade;
  });
                ...
                <SelectContent>
                  {filteredClasses.map((c) => (
                    <SelectItem key={c.id} value={c.id}>
                      {c.jenjang} - {c.name} ({c.studentCount} siswa)
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
              <div>
                <CardTitle className="text-lg">
                  {selectedClass?.jenjang} - {selectedClass?.name} - {selectedSubject?.name}
                </CardTitle>
