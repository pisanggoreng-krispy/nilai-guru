...
export default function WaliKelasPage() {
  ...
  const fetchClasses = async () => {
    try {
      // Use forWaliKelas parameter to filter classes for wali kelas only
      const res = await fetch('/api/classes?forWaliKelas=true');
      const data = await res.json();
      if (data.success) {
        setClasses(data.data);
        // Auto-select first class if only one (typical for wali kelas)
          ...
          <Card className="border-0 shadow-sm">
            <CardHeader className="pb-3">
              <CardTitle className="text-lg">
                Rekap Nilai - {recapData.class.jenjang} {recapData.class.name}
              </CardTitle>
              <CardDescription>
