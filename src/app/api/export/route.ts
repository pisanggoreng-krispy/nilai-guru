    ...
    switch (type) {
      ...
      case 'teachers': {
        const teacherData = (users || []).map(u => ({
          ...
          Email: u.email,
          Role: u.role,
          Password: '', // Empty for existing teachers - password not exposed
        }));
      ...
      case 'template-teachers': {
        // Template for importing teachers with password
        const templateData = [
          { Nama: 'Contoh Guru 1', Email: 'guru1@sekolah.com', Role: 'GURU_MAPEL', Password: 'Pass1234' },
          { Nama: 'Contoh Guru 2', Email: 'guru2@sekolah.com', Role: 'GURU_MAPEL', Password: '' }, // Empty = auto generate
          { Nama: 'Contoh Admin', Email: 'admin@sekolah.com', Role: 'ADMIN', Password: 'AdminPass1' },
        ];
        ...
        workbook = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(workbook, ws, 'Template Guru');
      ...
      case 'subjects': {
        ...
        if (grade) {
          filteredSubjects = filteredSubjects.filter(s => s.grade === parseInt(grade));
        }

        const subjectData = filteredSubjects.map(s => ({
          Nama: s.name,
          Jenjang: s.level || '',
          Kelas: s.grade || '',
        }));
        ...
        workbook = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(workbook, ws, 'Mata Pelajaran');
        
        if (level && grade) {
          filename = `mapel_${level}_${grade}.xlsx`;
        } else {
      ...
      case 'template-subjects': {
        // Export template for importing subjects
        // Only need Nama and Jenjang - grade will be taken from selected card
        const level = searchParams.get('level') || 'SMP';

        const templateData = [
          { Nama: 'Matematika', Jenjang: level },
          { Nama: 'Bahasa Indonesia', Jenjang: level },
          { Nama: 'Bahasa Inggris', Jenjang: level },
          { Nama: 'IPA', Jenjang: level },
          { Nama: 'IPS', Jenjang: level },
        ];
        ...
        workbook = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(workbook, ws, 'Template Mapel');
        filename = `template_mapel_${level}.xlsx`;
        break;
      }
      ...
      case 'leger': {
        ...
        // Check if user is wali kelas (by checking classes where waliKelasId matches user.id)
        if (!targetClass) {
        ...
        // Extract grade from class name or use class.grade field
        let classGrade = (targetClass as any).grade;
        if (!classGrade) {
          const gradeMatch = targetClass.name.match(/\d+/);
        ...
        const studentAverages = classStudents.map(student => {
          ...
          }).filter(g => g !== null && g !== undefined && !isNaN(g)) as number[];
