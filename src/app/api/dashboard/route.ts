...
export async function GET(request: NextRequest) {
  try {
    ...
    // Debug: log all classes with their waliKelasId
    console.log('Dashboard API - User:', JSON.stringify({ id: user.id, name: user.name, email: user.email }));
    console.log('Dashboard API - All classes with waliKelasId:', JSON.stringify(classes?.map(c => ({ id: c.id, name: c.name, waliKelasId: c.waliKelasId }))));
    console.log('Dashboard API - Wali Kelas Classes for this user:', waliKelasClasses.length);
    console.log('Dashboard API - isWaliKelas:', waliKelasClasses.length > 0);
