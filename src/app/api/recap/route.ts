...
export async function GET(request: NextRequest) {
  try {
    ...
    // Check permission - allow ADMIN and the wali kelas of this class
    // Also allow if user is a teacher (GURU_MAPEL) - they might be acting wali kelas
    const isAdmin = user.role === 'ADMIN';
    const isWaliKelas = targetClass.waliKelasId === user.id;
    
    if (!isAdmin && !isWaliKelas) {
      return NextResponse.json({ success: false, error: 'Tidak memiliki akses ke kelas ini' }, { status: 403 });
    }
    ...
    // Get subjects for this level AND grade
    ...
    if (classGrade) {
      subjectsQuery = subjectsQuery.eq('grade', classGrade);
    }
