...
// Delete user(s) - supports single or bulk delete
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (ids) {
      ...
      idsToDelete = ids.split(',').filter(Boolean);
    } else if (id) {
      // Single delete
      idsToDelete = [id];
    }
    ...
    if (idsToDelete.includes(decoded.userId)) {
    ...
    // Delete teacher_subjects relations
    await supabase
    ...
    const message = idsToDelete.length === 1 
      ...
      : `${idsToDelete.length} guru berhasil dihapus`;

    return NextResponse.json({ success: true, message, deletedCount: idsToDelete.length });
  } catch (error) {
